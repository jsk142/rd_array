...
std::string readDoubleQuotedString (std::istream &in)
{
    ...
    if ( ch != '\"' )
    {
        throw std::logic_error("ERROR: the opening quotation mark was not the first character");
    } // END if
    ...
    if (!in)
    {
        throw std::logic_error("ERROR: the closing quotation mark was not found");
    } // END if
